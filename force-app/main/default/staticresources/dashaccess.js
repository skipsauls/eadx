...
function getUser(share, callback) {
    
    console.warn("getUser; ", share);
    var config = {
        url: _baseUrl + "/query?q=SELECT+Id,Name+FROM+User+WHERE+Id+='" + share.sharedWithId + "'",
        method: "GET",
        headers: {
            "Content-Type": "application/json;charset=UTF-8"
        }    
    ...
    sendRequest(config, function(response) {
        ...
        if (response.status && (response.status < 200 || response.status > 299)) {
            if (response.response) {
                showToast("error", "Error: " + response.response[0].errorCode, response.response[0].message);
                if (typeof callback === "function") {
                    callback(share, null, {err: response.response[0].errorCode, msg: response.response[0].message});
                }
            }
        } else {
            if (typeof callback === "function") {
				callback(share, response.records, null);             
            }
		}
    });    
...
function getUsersInGroup(share, callback) {
    
    console.warn("getUsersInGroup; ", share);
    var config = {
        url: _baseUrl + "/query?q=SELECT+Id,Name+FROM+User+WHERE+Id+IN+(SELECT+UserOrGroupId+FROM+GroupMember+WHERE+GroupId='" + share.sharedWithId + "')",
        method: "GET",
        headers: {
            "Content-Type": "application/json;charset=UTF-8"
        }    
...
function getUsersInRole(share, callback) {
    console.warn("getUsersInRole; ", share);
    var config = {
        url: _baseUrl + "/query?q=SELECT+Id,Name+FROM+User+WHERE+UserRoleId='" + share.sharedWithId + "'",
        method: "GET",
        headers: {
            "Content-Type": "application/json;charset=UTF-8"
        }    
...
function updateAccessList(resType, resource, userMap) {
    console.warn("updateAccessList: ", resType, resource, userMap);
    var accessBody = document.querySelector("#access-table tbody");
    ...
    for (var id in userMap) {
        user = userMap[id];
        row = accessBody.querySelector("[data-user-id='" + id + "']");
        if (row === null || typeof row === "undefined") {
            ...
            cell.setAttribute("data-user-name", user.Name);
            ...
            cell = document.createElement("td");
            cell.innerHTML = "";
            cell.setAttribute("data-shared-type", "role");
            row.appendChild(cell);
            
            cell = document.createElement("td");
            cell.innerHTML = "";
            cell.setAttribute("data-shared-type", "group");
            row.appendChild(cell);

            cell = document.createElement("td");
            cell.innerHTML = null;
            cell.setAttribute("data-access-level", "access");
            row.appendChild(cell);
            
            accessBody.appendChild(row);
        }
        
        if (user.userAccessType) {
            row.classList.add("user-access-" + user.userAccessType);
	        cell = row.querySelector("[data-access-level='access']");
            accessType = user.userAccessType;
            if (accessType === "manage") {
                cell.innerHTML = accessType;
            } else if (cell.innerHTML === "view" && accessType === "edit") {
                cell.innerHTML = accessType;
            } else if (cell.innerHTML === null || cell.innerHTML === "") {
                cell.innerHTML = accessType;
            }
        }
        
        cell = row.querySelector("[data-shared-type='role']");
        cell.innerHTML = "";
        for (var roleName in user.roles) {
            cell.innerHTML += " " + user.roles[roleName].sharedWithLabel;
            row.classList.add("role-access-" + user.roles[roleName].accessType);
            accessType = user.roles[roleName].accessType;
            if (accessType === "manage") {
                cell.innerHTML = accessType;
            } else if (cell.innerHTML === "view" && accessType === "edit") {
                cell.innerHTML = accessType;
        ...
        cell = row.querySelector("[data-shared-type='group']");
        cell.innerHTML = "";
        for (var groupName in user.groups) {
            cell.innerHTML += " " + user.groups[groupName].sharedWithLabel;
            row.classList.add("group-access-" + user.groups[groupName].accessType);
            accessType = user.groups[groupName].accessType;
            if (accessType === "manage") {
                cell.innerHTML = accessType;
            } else if (cell.innerHTML === "view" && accessType === "edit") {
                cell.innerHTML = accessType;
...
function showAccess(resType, resource) {
    console.warn("showAccess: ", resType, resource);
    var accessBody = document.querySelector("#access-table tbody");
    accessBody.innerHTML = "";
    
    if (resource.type === "folder" || resource.folder) {
        var config = {
            url: resource.folder ? resource.folder.url : resource.url,
            method: "GET",
            headers: {
                "Content-Type": "application/json;charset=UTF-8"
            }    
            ...
            if (response.status && (response.status < 200 || response.status > 299)) {
                ...
                if (response.shares) {
                    ...
                    for (var i = 0; i < response.shares.length; i++) {
                        share = response.shares[i];
                        console.warn("share: ", share);
                        if (share.shareType === "role") {
                            getUsersInRole(share, function(share, users, err) {
                                console.warn("role users: ", users);
                                for (var i = 0; i < users.length; i++) {
                                    user = users[i];
                                    suser = userMap[user.Id];
                                    if (suser === null || typeof suser === "undefined") {
                                        suser = {
                                            Id: user.Id,
                                            Name: user.Name,
                                            url: user.attributes.url,
                                            userAccessType: share.accessType,
                                            groups: {},
                                            roles: {}
                                        };
                                        userMap[user.Id] = suser;
                                    }
                                    suser.roles[share.sharedWithId] = share;
                                }
                                updateAccessList(resType, resource, userMap);
                            });
                        } else if (share.shareType === "group") {
                            getUsersInGroup(share, function(share, users, err) {
                                console.warn("group users: ", users);
                                for (var i = 0; i < users.length; i++) {
                                    user = users[i];
                                    suser = userMap[user.Id];
                                    if (suser === null || typeof suser === "undefined") {
                                    ...
                                    suser.groups[share.sharedWithId] = share;
                                }
                                updateAccessList(resType, resource, userMap);                                
                            });                            
                        } else {
                            // User share
                            getUser(share, function(share, users, err) {
                                console.warn("user users: ", users);
                                for (var i = 0; i < users.length; i++) {
                                    user = users[i];
                                    suser = userMap[user.Id];
                                    if (suser === null || typeof suser === "undefined") {
