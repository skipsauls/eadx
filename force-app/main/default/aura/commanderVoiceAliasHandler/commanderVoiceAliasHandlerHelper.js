({
    handleCommanderEventChange: function(component, event, helper) {
        let self = this;
        let err = null;
        let payload = component.get('v.commanderEvent');
        
        if (err) {
            let error = JSON.parse(JSON.stringify(err));
            console.error('handleCommanderEventChange error: ', err);
        } else {
            let items = null;
            let error = null;
            ...
            if (action.type === 'InvocableActionApex') {
    ...
    handleVoiceAlias: function(component, payload) {
      	let self = this;
        let action = payload.action;
        let response = payload.response;
        let parameters = payload.parameters;
                        ...
                        if (bestMatch) {
                            ...
                            self.execute(component, bestMatch, function(err, results) {
                                ...
                                self.getCommanderTemplate(component, bestMatch.name, function(err, template) {
                                    ...
                                    component.set('v.speechText', result);
						            component.set('v.systemLogText', result);
